...
const postFavourite = async (req, res, next) => {
    try {
        ...
        if (!favouritesByUser) {
            ...
            const populatedNewFavourites = await Favourites.findById(newFavourites._id).populate('user').populate('dishes');
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json')
        ...
        const populatedNewFavourites = await Favourites.findById(favouritesByUser._id).populate('user').populate('dishes');
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json')
        res.json(populatedNewFavourites);
    } catch (e) {
...
const getFavouritesByParam = async (req, res, next) => {
    try {
        ...
        if (favourites.dishes.contains(req.params.id)) {
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
...
const postFavouritesByParam = async (req, res, next) => {
    try {
        ...
        if (!favourite) {
            ...
            const populatedFavourites = await Favourites.findById(newFavourites._id).populate('user').populate('dishes');
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json')
...
const deleteFavouritesByParam = async (req, res, next) => {
    ...
    if (!favourite.dishes.contains(req.params.id)) {
    ...
    favourite.dishes.splice(favourite.dishes.indexOf(req.params.id), 1);
    await favourite.save();
    const populatedFavourite = Favourites.findById(favourite._id).populate('user').populate('dishes');
    res.statusCode = 200;
    res.setHeader('Content-Type', 'application/json');
